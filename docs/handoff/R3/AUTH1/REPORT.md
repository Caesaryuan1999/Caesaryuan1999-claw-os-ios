...
- Secondary original-account login retains existing username normalization and password bytes. The follow-up LEGACY-COMPAT closes the original d077e02 dependency on AUTH HTTP: after an attempted capability check, missing/404/failed AUTH permits basic; any parsed legacy_basic=false remains authoritative, including enabled=false responses. New phone/email login still requires supported AUTH. A 3xx response shows administrator recovery and never stock OTP. The legacy Credentials branch remains inherited, with no new UI caller.
...
## Follow-up legacy compatibility

See LEGACY-COMPAT.md for the independently committed two-file production correction requested by root. Native AUTH methods are now 26 (expected cumulative 97); actual Swift/Mac execution remains NOT_RUN here. Original static/scope JSON files document d077e02, not the follow-up hash.
