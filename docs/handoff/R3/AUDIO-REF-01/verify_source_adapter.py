...
assert source == before + anchor + after.replace(old_body, old_body.replace("refurl: ref,", "refurl: srvUrl,"), 1), "Production delta must be exactly the approved success argument"
...
for name, code, dirty, expected in [
    ("legacy-red", old_source, False, "Completed upload must use its server result"),
...
checks = {"scope": "Actual CI extractor executed against real safe source Git snapshots; not Swift/Drafty runtime",
          "production_delta": "one successful audio ref argument only",
          "checks": results, "native_execution": "NOT_RUN; two methods prepared for later exact Mac CI"}
(REPORT / "source-adapter-checks.json").write_text(json.dumps(checks, indent=2) + "\n", encoding="utf-8")
