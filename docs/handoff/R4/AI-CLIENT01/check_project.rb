...
expected = {
  'Tinodios' => %w[ClawAssistantModels.swift ClawAssistantService.swift ClawAssistantSession.swift
    ...
    ClawAssistantConversationViewController.swift].sort,
  'TinodiosVoiceLayoutTests' => %w[AssistantHistoryTests.swift AssistantLayoutTests.swift].sort
}
raise 'Unexpected Assistant source membership' unless membership == expected
puts JSON.pretty_generate({ scope: 'Real Xcodeproj parser; not Xcode build/link',
