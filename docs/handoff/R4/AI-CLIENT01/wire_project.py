"""One-time, exact-anchor PBX/selector edit for the authorized A sources."""
from pathlib import Path

project = Path('Tinodios.xcodeproj/project.pbxproj')
text = project.read_text(encoding='utf-8')
production = ['ClawAssistantModels', 'ClawAssistantService', 'ClawAssistantSession',
              'ClawAssistantHistory', 'ClawAssistantViewController',
              'ClawAssistantHistoryViewController', 'ClawAssistantConversationViewController']
tests = ['AssistantHistoryTests', 'AssistantLayoutTests']
names = production + tests
refs = {name: f'C1A07000000000000000{index * 2 + 1:04X}' for index, name in enumerate(names)}
builds = {name: f'C1A07000000000000000{index * 2 + 2:04X}' for index, name in enumerate(names)}
...
anchor = 'C1A060000000000000000002, ); runOnlyForDeploymentPostprocessing = 0;'
replace(anchor, 'C1A060000000000000000002, ' + ', '.join(builds[name] for name in tests) +
        ', ); runOnlyForDeploymentPostprocessing = 0;')
project.write_text(text, encoding='utf-8', newline='\n')
script = Path('Scripts/ci/verify_publish_outcomes_macos.sh')
text = script.read_text(encoding='utf-8')
anchor = '  -only-testing:TinodiosVoiceLayoutTests/CoreListLayoutTests \\\n'
replace(anchor, anchor + ''.join(f'  -only-testing:TinodiosVoiceLayoutTests/{name} \\\n' for name in tests))
script.write_text(text, encoding='utf-8', newline='\n')
print('7 App sources + 2 existing App-hosted test sources; 2 selectors appended')
