...
def main() -> None:
    ...
    assert "已退出登录" in account and "logoutAndRouteToLoginVC(ifCurrent: owner)" in account
    assert "Cache.sessionGeneration == generation, owner.myUid == uid, Cache.isCurrent(owner)" in account
